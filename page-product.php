...
<main id="main" class="site-main" stlye="display:block;">
	<!-- <iframe src="<?php echo 'https://secure.foreshop.net/product/'.$url.'?wpam_id='.$wpam_id; ?>" style="width:100%;height:1850px" frameborder="0"></iframe> -->
    <iframe id="__product" src="<?php echo 'https://secure.foreshop.net/products/?product_id=' . $product_id . '&' . 'wpam_id=' . $wpam_id . '&website=' . site_url(); ?>" scrolling="no" style="width:100%;" frameborder="0" sandbox="allow-top-navigation allow-same-origin allow-scripts allow-forms"></iframe>

</main>

<div class="position-fixed f-subscribe bg-light" style="padding-left:4em!important;" id="coupon-side-subscribe">
  <button type="button" id="side-pop" class="close position-absolute" aria-label="Close" ><span aria-hidden="true">×</span></button>
...
<div class="f-modal-coupon f-modal modal fade" style="background-color: #00000099;">
  <div class="f-coupon-modal modal-dialog modal-dialog-centered">
    <div class="modal-content text-center px-3 py-2 ">
      ...
      <h3 id="big-pop" class="close">NO THANKS I'LL JUST KEEP SHOPPING</h3>
    </div>
  </div>
</div>
...
$(document).ready(function(){
  ...
  var _productDOM = $('iframe#__product').contents();
  var product_link = _productDOM.find('._related-products-container');

  console.log(product_link.html());
});
</script> -->
...
<script>
  ...
  function data(e) {
    console.log(e);
    let iframe = document.querySelector('#__product');
    if (e.origin === 'https://secure.foreshop.net') {
      let elementsData = e.data;
      
      iframe.height = elementsData+ 200 + "px";
